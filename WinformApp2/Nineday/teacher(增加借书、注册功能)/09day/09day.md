...
- `select * from 表名 where 条件`
  ...
  - 条件: `=   !=  >   >=    <   <=     in    between 数字 and 数字   like    `
    - 条件合并: `and   or`
  - 聚合函数: `select count(*) as count,sum(*),avg(*),max(*),min(*) from 表名`
  - 排序分组
...
```C#
// 定义连接数据库字符串
string connStr = server=ip地址;port=3306;database=数据库;uid=登录名;password=密码;charset=字符集;
// 连接数据库 
using(MysqlConnection coon = new MysqlConnection(连接字符串)){
    // 打开连接
    coon.Open();
    string sql = "....";
    using(MysqlCommand Cmd=new MysqlCommand(sql,coon)){
        ...
        // 非查询
        int rows =Cmd.ExcuteNonQuery() // rows 执行影响的行数
    }
