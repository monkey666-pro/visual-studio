...
namespace Day15
{
    internal class car_rent//管理车俩信息
    {
        ...
        public void rentcount()
        { 
            if(!File.Exists(Path)) Console.WriteLine("无记录");
            List<rentreturn> list = new();
            var json=File.ReadAllText(Path);
            var res = JsonSerializer.Deserialize<List<rentreturn>>(json);
            foreach (rentreturn i in res)
            {
                Console.WriteLine($"id:{i.Id}   车id：{i.Carid}   客户id：{i.Costomerid}   借车时间：{i.Renttime}  还车：{i.Returntime}   费用：{i.Hourcount}  ");
            }
